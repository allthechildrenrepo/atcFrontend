...
import { LeadFetchService, LeadRequestService, AssignBaseService } from '../../shared/services/leads';
...
export class BaseComponent extends BasePage implements OnInit {
  ...
  requestForBase() {
    ...
    let params = this.leadRequestService.getParams(this.form.value.number, this.selectedBranchId);
    this.leadRequestService.post(params).subscribe((data) => {
  ...
  assignBase() {
    ...
    this.assignBaseService.put(assignBaseParams).subscribe(
      data => {
        this.dismissLoader();
        this.showAlert("success", 'Base assigned Successfully for ' + this.selectTelecaller.fullName);
